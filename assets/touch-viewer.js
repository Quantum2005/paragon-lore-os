...
fileLabel.textContent = `TOUCH://${filename || "UNKNOWN"}`;
...
const fetchFile = async () => {
  ...
  try {
    ...
    viewer.value = String(payload.file?.content || "");
    viewer.setSelectionRange(0, 0);
    showStatus("READ-ONLY VIEW READY. PRESS ESC TO RETURN.", false);
  } catch (_error) {
    showStatus("TOUCH LOAD FAILED [E_API_UNAVAILABLE] API UNAVAILABLE", true);
  }
};
